(function () {
  ...
  function applyFormalState(stateSource) {
    ...
    var status = "W01-D03 是十二周长期线的每日课程页；当前正式主线仍以 study-project.json 为准。";
    ...
    var condition = practiceReady
      ? "W01-D03 页面四项学习活动已完成；下一步是在 Codex 中确认是否把本日课纳入长期线证据。"
      : "先完成 W01-D03 的五个模块与 Prompt 任务契约工作坊；页面活动不会写入正式 JSON。";

    setText("[data-later-page-status]", status);
    setText("[data-later-cockpit-title]", "W01-D03 日课：把 Prompt 产品作用接到 Agent 任务契约。");
    ...
    setText("[data-later-current-gap]", "今天要把 LLM 的生成能力对照 AI 面试与项目答辩教练 Agent，形成Prompt 任务契约。");
    setText("[data-later-formal-task]", "完成 W01-D03 Prompt 任务契约草稿；不写入正式状态源");
    setText("[data-later-current-surface]", "网页 · 十二周长期线 W01-D03");
    setText("[data-later-switch-condition]", condition);
    setText("[data-later-sticky-surface]", "网页 · W01-D03 日课");
    setText("[data-later-sticky-condition]", condition);
    setText("[data-later-formal-summary]", formalProgress);
    setText("[data-formal-handoff-title]", "W01-D03 接力：页面完成后，再决定如何记录长期线证据。");
    setText("[data-formal-handoff-summary]", "网页里的概念、练习、笔记和 Prompt 任务契约只是草稿证据。长期线是否正式开启、如何记录 W01-D03，需要 Codex 重新读取并写入唯一正式状态源。");
    setText("[data-formal-handoff-primary]", practiceReady ? "核对我的 W01-D03 理解地图" : "继续 W01-D03 日课");
    setText("[data-copy-formal-entry]", "复制 W01-D03 学习接力");
    ...
    window.__STUDY_PAGE_HANDOFF__ = {
      ...
      formal_scope: "本页不创建 session、pending question、评分、复测关系或掌握证据；只生成 W01-D03 Prompt 任务契约草稿。",
      reteach_contract: "若模型任务契约只停留在能力罗列，请按‘任务 → 模型贡献 → 禁止事项 → 校验/确认 → 失败兜底’重新收敛。"
    };
  }
  ...
  function setupFinalChallenge() {
    ...
    form.addEventListener("submit", function (event) {
      ...
      if (error) error.textContent = completed ? "" : "请沿六步路线补充 W01-D03 Prompt 任务契约，并完成五项覆盖核对。";
      if (feedback) feedback.hidden = !completed;
      refreshProgressAndHandoff();
      saveState(completed ? "W01-D03 Prompt 任务契约已保存；可以查看示例。" : "W01-D03 任务契约还没有覆盖完整链路。");
    });
  }
  ...
  function answerCardMarkdown() {
    ...
    var lines = ["# 我的 W01-D03 Prompt 产品作用理解地图", ""];
    moduleOrder.forEach(function (id) {
      lines.push("## " + (moduleLabels[id] || id), answer(id + "_teachback", "还没有记录这一模块的学习笔记"), "");
    });
    lines.push("## W01-D03 Prompt 任务契约", answer("final_diagnosis", "还没有完成 W01-D03 综合工作坊"), "");
  ...
  function setupAnswerCard() {
    ...
    if (build) build.addEventListener("click", function () {
      ...
      saveState("W01-D03 理解地图已由你的笔记和Prompt 任务契约汇总。");
    });
  ...
  function setupActions() {
    ...
    document.querySelectorAll("[data-copy-formal-entry]").forEach(function (button) {
      button.addEventListener("click", function () {
        copyText(JSON.stringify(window.__STUDY_PAGE_HANDOFF__ || {}, null, 2), "W01-D03 学习接力已复制。");
      });
    });
  }
  ...
  if (state.migrated_legacy_key) saveState("旧 W01-D03 草稿已迁移到新的学习闭环；原数据未删除。");
})();
