(function () {
  ...
  function applyFormalState(stateSource) {
    ...
    var status = "W5 是十二周长期线的证据驱动评分页；当前正式主线仍以 study-project.json 为准。";
    ...
    var condition = practiceReady
      ? "W5 页面四项学习活动已完成；下一步是在 Codex 中确认是否进入正式评测或继续长期线。"
      : "先完成 W5 的六个证据评分机制模块与综合评分报告工作坊；页面活动不会写入正式 JSON。";

    setText("[data-later-page-status]", status);
    setText("[data-later-cockpit-title]", "W5 证据评分：让分数、结论和置信表达都能回到原话。");
    setText("[data-later-cockpit-summary]", formalProgress + "网页只记录当前标签页草稿，不创建长期线正式掌握。");
    setText("[data-later-cockpit-mode]", "WEEK 05 PREVIEW");
    ...
    setText("[data-later-current-gap]", "W5 需要把 Rubric、证据抽取、分数裁决、信息不足、置信表达和评分报告放进同一条可追溯评分链。");
    setText("[data-later-formal-task]", "制定 W5 证据驱动评分报告草稿；不写入正式状态源");
    setText("[data-later-current-surface]", "网页 · 十二周长期线 W5");
    setText("[data-later-switch-condition]", condition);
    setText("[data-later-sticky-surface]", "网页 · W5 证据驱动评分");
    setText("[data-later-sticky-condition]", condition);
    setText("[data-later-formal-summary]", formalProgress);
    setText("[data-formal-handoff-title]", "W5 接力：页面完成后，再决定是否启动正式评测。");
    setText("[data-formal-handoff-summary]", "网页里的评分判断、Rubric 草稿、笔记和理解地图只是草稿证据。正式长期线是否记录 Week 05，需要 Codex 重新读取并写入唯一正式状态源。");
    setText("[data-formal-handoff-primary]", practiceReady ? "核对我的 W5 理解地图" : "继续 W5 评分报告");
    setText("[data-copy-formal-entry]", "复制 W5 证据评分接力");

    setText('[data-later-stage-status="foundation"]', "W1 产物与两周突击线记录可作为输入");
    setText('[data-later-stage-status="week01"]', "作为长期线基础输入，不在本页改写");
    setText('[data-later-stage-status="week02"]', "作为 W5 评分链前置输入，不在本页改写");
    setText('[data-later-stage-status="formal"]', "正式状态源未由本页改写");

    window.__STUDY_PAGE_HANDOFF__ = {
      track: "twelve_week_depth",
      week_id: config.week_id || "week-05",
      focus: config.day_label + " / " + config.knowledge_anchor,
      page_anchor: config.page_anchor,
      current_formal_state: formalProgress,
      next_surface: "Codex · 是否记录 Week 05 课程进程或启动正式评测",
      switch_condition: condition,
      formal_scope: "本页不创建 session、pending question、评分、错题、复测关系或掌握证据；只生成 W5 证据驱动评分报告草稿。",
      reteach_contract: "若 W5 方案只停留在“让模型给分”，请按‘Rubric → 原话证据 → 分数裁决 → 信息不足 → 置信表达 → 可追溯报告’重新收敛。"
    };
  }
  ...
  function setupFinalChallenge() {
    ...
    form.addEventListener("submit", function (event) {
      ...
      if (error) error.textContent = completed ? "" : "请沿五步路线补充 W5 证据驱动评分报告，并完成五项覆盖核对。";
      if (feedback) feedback.hidden = !completed;
      refreshProgressAndHandoff();
      saveState(completed ? "W5 综合评分报告已保存；可以查看示例。" : "W5 评分报告还没有覆盖完整链路。");
    });
  }
  ...
  function answerCardMarkdown() {
    ...
    var lines = ["# 我的 W5 证据驱动评分理解地图", ""];
    moduleOrder.forEach(function (id) {
      lines.push("## " + (moduleLabels[id] || id), answer(id + "_teachback", "还没有记录这一模块的学习笔记"), "");
    });
    lines.push("## W5 综合评分报告", answer("final_diagnosis", "还没有完成 W5 综合工作坊"), "");
  ...
  function setupAnswerCard() {
    ...
    if (build) build.addEventListener("click", function () {
      ...
      saveState("W5 理解地图已由你的笔记和评分报告草稿汇总。");
    });
  ...
  function setupActions() {
    ...
    document.querySelectorAll("[data-copy-formal-entry]").forEach(function (button) {
      button.addEventListener("click", function () {
        copyText(JSON.stringify(window.__STUDY_PAGE_HANDOFF__ || {}, null, 2), "W5 证据评分接力已复制。");
      });
    });
  }
  ...
  if (state.migrated_legacy_key) saveState("旧 W5 草稿已迁移到新的学习闭环；原数据未删除。");
})();
