(function () {
  ...
  function applyFormalState(stateSource) {
    ...
    var status = "W15 是十二周长期线之后的团队级 AI 产品运营延展页；当前正式主线仍以 study-project.json 为准。";
    ...
    var condition = practiceReady
      ? "W15 页面四项学习活动已完成；下一步是在 Codex 中确认是否记录团队运营机制草稿。"
      : "先完成 W15 的六个团队运营模块与综合运营复盘工作坊；页面活动不会写入正式 JSON。";

    setText("[data-later-page-status]", status);
    setText("[data-later-cockpit-title]", "W15 团队运营：把早期 AI 项目变成可持续复盘、增长和治理的产品机制。");
    setText("[data-later-cockpit-summary]", formalProgress + "网页只记录当前标签页草稿，不创建真实增长、团队绩效或长期线正式掌握。");
    setText("[data-later-cockpit-mode]", "WEEK 15 EXTENSION");
    setText("[data-later-cockpit-updated]", formatDate(lastProjectState.updated_at));
    setText("[data-later-passed-scope]", "承接 W14 的入职 30-60-90 天落地计划；不重写十二周主线结论。");
    setText("[data-later-current-gap]", "W15 需要把运营节奏、增长实验、质量监控、成本容量、治理决策和路线图复盘放进同一条团队工作机制。");
    setText("[data-later-formal-task]", "制定 W15 AI 产品团队运营机制草稿；不写入正式状态源");
    setText("[data-later-current-surface]", "网页 · 十二周长期线延展 W15");
    setText("[data-later-switch-condition]", condition);
    setText("[data-later-sticky-surface]", "网页 · W15 AI 产品团队运营机制");
    setText("[data-later-sticky-condition]", condition);
    setText("[data-later-formal-summary]", formalProgress);
    setText("[data-formal-handoff-title]", "W15 接力：页面完成后，再决定是否记录团队运营机制草稿。");
    setText("[data-formal-handoff-summary]", "网页里的运营节奏、增长实验、质量监控、成本容量、治理决策、路线图复盘和理解地图只是草稿证据。正式状态是否记录 Week 15 延展机制，需要 Codex 重新读取并写入唯一正式状态源。");
    setText("[data-formal-handoff-primary]", practiceReady ? "核对我的 W15 理解地图" : "继续 W15 运营机制");
    setText("[data-copy-formal-entry]", "复制 W15 团队运营接力");

    setText('[data-later-stage-status="foundation"]', "两周突击线与 W1-W12 产物可作为输入");
    setText('[data-later-stage-status="week12"]', "W14 入职计划可作为 W15 团队运营前置输入");
    setText('[data-later-stage-status="evidence"]', "真实数据、团队反馈、实验假设和网页草稿需要分层标注");
    setText('[data-later-stage-status="formal"]', "正式状态源未由本页改写");

    window.__STUDY_PAGE_HANDOFF__ = {
      track: "twelve_week_depth_extension",
      week_id: config.week_id || "week-15",
      focus: config.day_label + " / " + config.knowledge_anchor,
      page_anchor: config.page_anchor,
      current_formal_state: formalProgress,
      next_surface: "Codex · 是否记录 Week 15 AI 产品团队运营机制草稿",
      switch_condition: condition,
      formal_scope: "本页不创建 session、pending question、评分、错题、复测关系、真实增长、团队绩效、线上指标、管理审批、客户背书、商业结果或掌握证据；只生成 W15 AI 产品团队运营机制草稿。",
      reteach_contract: "若 W15 计划只停留在“持续优化、继续增长、加强治理”，请按‘Operating Cadence → Growth Experiment → Quality Monitor → Cost Capacity → Governance Review → Roadmap Portfolio’重新收敛。"
    };
  }
  ...
  function setupFinalChallenge() {
    ...
    form.addEventListener("submit", function (event) {
      ...
      if (error) error.textContent = completed ? "" : "请沿五步路线补充 W15 团队运营机制，并完成五项覆盖核对。";
      if (feedback) feedback.hidden = !completed;
      refreshProgressAndHandoff();
      saveState(completed ? "W15 团队运营机制已保存；可以查看示例。" : "W15 团队运营机制还没有覆盖完整链路。");
    });
  }
  ...
  function answerCardMarkdown() {
    ...
    var lines = ["# 我的 W15 AI 产品团队运营机制理解地图", ""];
    moduleOrder.forEach(function (id) {
      lines.push("## " + (moduleLabels[id] || id), answer(id + "_teachback", "还没有记录这一模块的学习笔记"), "");
    });
    lines.push("## W15 团队运营综合机制", answer("final_diagnosis", "还没有完成 W15 综合工作坊"), "");
    lines.push("> 这是网页学习草稿，不是真实增长、线上指标、团队绩效、管理审批、客户背书、商业结果或正式掌握证据；真实数据、团队反馈、实验假设和网页草稿应分层标注。");
    return lines.join("\n");
  }
  ...
  function setupAnswerCard() {
    ...
    if (build) build.addEventListener("click", function () {
      ...
      saveState("W15 理解地图已由你的笔记和运营机制草稿汇总。");
    });
  ...
  function setupActions() {
    ...
    document.querySelectorAll("[data-copy-formal-entry]").forEach(function (button) {
      button.addEventListener("click", function () {
        copyText(JSON.stringify(window.__STUDY_PAGE_HANDOFF__ || {}, null, 2), "W15 团队运营接力已复制。");
      });
    });
  }
  ...
  if (state.migrated_legacy_key) saveState("旧 W15 草稿已迁移到新的学习闭环；原数据未删除。");
})();
