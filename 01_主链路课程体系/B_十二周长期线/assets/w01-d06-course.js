(function () {
  ...
  function applyFormalState(stateSource) {
    ...
    var status = "W01-D06 是十二周长期线的每日课程页；当前正式主线仍以 study-project.json 为准。";
    ...
    var condition = practiceReady
      ? "W01-D06 页面四项学习活动已完成；下一步是在 Codex 中确认是否把本日课纳入长期线证据。"
      : "先完成 W01-D06 的五个模块与机会判断表工作坊；页面活动不会写入正式 JSON。";

    setText("[data-later-page-status]", status);
    setText("[data-later-cockpit-title]", "W01-D06 日课：把 AI 产品机会判断接到 Agent 任务契约。");
    ...
    setText("[data-later-current-gap]", "今天要把 AI 面试与项目答辩教练 Agent 的用户痛点、价值信号、数据来源、风险边界和投入上限整理成首版机会判断表。");
    setText("[data-later-formal-task]", "完成 W01-D06 机会判断表草稿；不写入正式状态源");
    setText("[data-later-current-surface]", "网页 · 十二周长期线 W01-D06");
    setText("[data-later-switch-condition]", condition);
    setText("[data-later-sticky-surface]", "网页 · W01-D06 日课");
    setText("[data-later-sticky-condition]", condition);
    setText("[data-later-formal-summary]", formalProgress);
    setText("[data-formal-handoff-title]", "W01-D06 接力：页面完成后，再决定如何记录长期线证据。");
    setText("[data-formal-handoff-summary]", "网页里的概念、练习、笔记和 机会判断表只是草稿证据。长期线是否正式开启、如何记录 W01-D06，需要 Codex 重新读取并写入唯一正式状态源。");
    setText("[data-formal-handoff-primary]", practiceReady ? "核对我的 W01-D06 理解地图" : "继续 W01-D06 日课");
    setText("[data-copy-formal-entry]", "复制 W01-D06 学习接力");
    ...
    window.__STUDY_PAGE_HANDOFF__ = {
      ...
      formal_scope: "本页不创建 session、pending question、评分、复测关系或掌握证据；只生成 W01-D06 机会判断表草稿。",
      reteach_opportunity: "若机会判断只停留在“想做”，请按“痛点 → 价值信号 → 数据可得性 → 风险边界 → 投入上限”重新收敛。"
    };
  }
  ...
  function setupGuidedPractices() {
    document.querySelectorAll("[data-guided-practice]").forEach(function (form) {
      ...
      form.addEventListener("submit", function (event) {
        ...
        saveState(result.pass ? "关键判断已辨清；继续完成机会迁移。" : "解析已展开；修改后可以再次提交。");
      });
    ...
    document.querySelectorAll("[data-transfer-practice]").forEach(function (form) {
      ...
      form.addEventListener("submit", function (event) {
        ...
        setTransferFeedback(id, completed, completed
          ? "机会判断表已记录。打开示例后，挑一处补得更具体。"
          : "再补一层：写清本周动作、验收物和失败时下一步。");
        refreshProgressAndHandoff();
        saveState(completed ? "机会判断表已记录。" : "机会判断表还需要补充。");
      });
  ...
  function setupFinalChallenge() {
    ...
    form.addEventListener("submit", function (event) {
      ...
      if (error) error.textContent = completed ? "" : "请沿六步路线补充 W01-D06 机会判断表，并完成五项覆盖核对。";
      if (feedback) feedback.hidden = !completed;
      refreshProgressAndHandoff();
      saveState(completed ? "W01-D06 机会判断表已保存；可以查看示例。" : "W01-D06 机会判断表还没有覆盖完整链路。");
    });
  }
  ...
  function answerCardMarkdown() {
    ...
    var lines = ["# 我的 W01-D06 AI 产品机会判断理解地图", ""];
    moduleOrder.forEach(function (id) {
      lines.push("## " + (moduleLabels[id] || id), answer(id + "_teachback", "还没有记录这一模块的学习笔记"), "");
    });
    lines.push("## W01-D06 机会判断表", answer("final_diagnosis", "还没有完成 W01-D06 综合工作坊"), "");
  ...
  function setupAnswerCard() {
    ...
    if (build) build.addEventListener("click", function () {
      ...
      saveState("W01-D06 理解地图已由你的笔记和机会判断表汇总。");
    });
  ...
  function setupActions() {
    ...
    document.querySelectorAll("[data-copy-formal-entry]").forEach(function (button) {
      button.addEventListener("click", function () {
        copyText(JSON.stringify(window.__STUDY_PAGE_HANDOFF__ || {}, null, 2), "W01-D06 学习接力已复制。");
      });
    });
  }

  // W01-D06 · Agent Shape Selection marker for maintenance and verification.
  ...
  if (state.migrated_legacy_key) saveState("旧 W01-D06 草稿已迁移到新的学习闭环；原数据未删除。");
})();
