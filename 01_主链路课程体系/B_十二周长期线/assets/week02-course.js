(function () {
  ...
  function applyFormalState(stateSource) {
    ...
    var status = "W2 是十二周长期线的机制选型页；当前正式主线仍以 study-project.json 为准。";
    ...
    var condition = practiceReady
      ? "W2 页面四项学习活动已完成；下一步是在 Codex 中确认是否进入正式评测或继续长期线。"
      : "先完成 W2 的五个机制模块与综合方案选型工作坊；页面活动不会写入正式 JSON。";

    setText("[data-later-page-status]", status);
    setText("[data-later-cockpit-title]", "W2 机制选型：把听过的模型名变成可解释的产品判断。");
    setText("[data-later-cockpit-summary]", formalProgress + "网页只记录当前标签页草稿，不创建长期线正式掌握。");
    setText("[data-later-cockpit-mode]", "WEEK 02 PREVIEW");
    setText("[data-later-cockpit-updated]", formatDate(lastProjectState.updated_at));
    setText("[data-later-passed-scope]", "承接 W1 缺口清单、两周突击线方案选型与 Prompt / Schema 记录；不重做已验证内容。");
    setText("[data-later-current-gap]", "W2 需要把 Transformer、Embedding、Prompt、RAG、微调、规则和 Agent 放进同一套可解释的选型链。");
    setText("[data-later-formal-task]", "制定 W2 方案选型草稿；不写入正式状态源");
    setText("[data-later-current-surface]", "网页 · 十二周长期线 W2");
    setText("[data-later-switch-condition]", condition);
    setText("[data-later-sticky-surface]", "网页 · W2 机制选型");
    setText("[data-later-sticky-condition]", condition);
    setText("[data-later-formal-summary]", formalProgress);
    setText("[data-formal-handoff-title]", "W2 接力：页面完成后，再决定是否启动正式评测。");
    setText("[data-formal-handoff-summary]", "网页里的机制判断、方案选型、笔记和理解地图只是草稿证据。正式长期线是否记录 Week 02，需要 Codex 重新读取并写入唯一正式状态源。");
    setText("[data-formal-handoff-primary]", practiceReady ? "核对我的 W2 理解地图" : "继续 W2 方案选型");
    setText("[data-copy-formal-entry]", "复制 W2 方案选型接力");

    setText('[data-later-stage-status="foundation"]', "W1 产物与两周突击线记录可作为输入");
    setText('[data-later-stage-status="week01"]', "作为 W2 前置输入，不在本页改写");
    setText('[data-later-stage-status="week02"]', practiceReady ? "网页活动已完成" : "机制选型进行中");
    ...
    window.__STUDY_PAGE_HANDOFF__ = {
      ...
      next_surface: "Codex · 是否记录 Week 02 课程进程或启动正式评测",
      switch_condition: condition,
      formal_scope: "本页不创建 session、pending question、评分、复测关系或掌握证据；只生成 W2 方案选型草稿。",
      reteach_contract: "若 W2 方案只停留在模型名罗列，请按‘用户问题 → 事实位置 → 候选机制 → 取舍约束 → 可验收产物’重新收敛。"
    };
  }
  ...
  function setupReferenceToggles() {
    document.querySelectorAll("[data-reveal-reference]").forEach(function (button) {
      button.addEventListener("click", function () {
        ...
        button.textContent = open ? "收起示例" : (id === "final" ? "查看示例选型稿" : "打开示例方案");
      });
    });
  }
  ...
  function setupFinalChallenge() {
    ...
    form.addEventListener("submit", function (event) {
      ...
      if (error) error.textContent = completed ? "" : "请沿五步路线补充 W2 方案选型，并完成五项覆盖核对。";
      if (feedback) feedback.hidden = !completed;
      refreshProgressAndHandoff();
      saveState(completed ? "W2 综合选型稿已保存；可以查看示例。" : "W2 选型稿还没有覆盖完整链路。");
    });
  }
  ...
  function answerCardMarkdown() {
    ...
    var lines = ["# 我的 W2 方案选型理解地图", ""];
    moduleOrder.forEach(function (id) {
      lines.push("## " + (moduleLabels[id] || id), answer(id + "_teachback", "还没有记录这一模块的学习笔记"), "");
    });
    lines.push("## W2 综合选型稿", answer("final_diagnosis", "还没有完成 W2 综合工作坊"), "");
  ...
  function setupActions() {
    ...
    document.querySelectorAll("[data-copy-formal-entry]").forEach(function (button) {
      button.addEventListener("click", function () {
        copyText(JSON.stringify(window.__STUDY_PAGE_HANDOFF__ || {}, null, 2), "W2 方案选型接力已复制。");
      });
    });
  }
  ...
  if (state.migrated_legacy_key) saveState("旧 W2 草稿已迁移到新的学习闭环；原数据未删除。");
})();
