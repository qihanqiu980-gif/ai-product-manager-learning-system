(function () {
  ...
  function applyFormalState(stateSource) {
    ...
    var status = "W3 是十二周长期线的 RAG 知识系统页；当前正式主线仍以 study-project.json 为准。";
    ...
    var condition = practiceReady
      ? "W3 页面四项学习活动已完成；下一步是在 Codex 中确认是否进入正式评测或继续长期线。"
      : "先完成 W3 的六个 RAG 机制模块与综合知识库工作坊；页面活动不会写入正式 JSON。";

    setText("[data-later-page-status]", status);
    setText("[data-later-cockpit-title]", "W3 RAG 知识系统：把资料变成可引用、可更新、可降级的答案链。");
    setText("[data-later-cockpit-summary]", formalProgress + "网页只记录当前标签页草稿，不创建长期线正式掌握。");
    setText("[data-later-cockpit-mode]", "WEEK 03 PREVIEW");
    setText("[data-later-cockpit-updated]", formatDate(lastProjectState.updated_at));
    setText("[data-later-passed-scope]", "承接 W2 的机制选型和两周突击线 Day 8 相关项目骨架；不重做已验证内容。");
    setText("[data-later-current-gap]", "W3 需要把资料源、切片、向量索引、检索、重排、引用和更新放进同一条可解释的 RAG 产品链。");
    setText("[data-later-formal-task]", "制定 W3 RAG 知识库方案草稿；不写入正式状态源");
    setText("[data-later-current-surface]", "网页 · 十二周长期线 W3");
    setText("[data-later-switch-condition]", condition);
    setText("[data-later-sticky-surface]", "网页 · W3 RAG 知识系统");
    setText("[data-later-sticky-condition]", condition);
    setText("[data-later-formal-summary]", formalProgress);
    setText("[data-formal-handoff-title]", "W3 接力：页面完成后，再决定是否启动正式评测。");
    setText("[data-formal-handoff-summary]", "网页里的 RAG 判断、知识库方案、笔记和理解地图只是草稿证据。正式长期线是否记录 Week 03，需要 Codex 重新读取并写入唯一正式状态源。");
    setText("[data-formal-handoff-primary]", practiceReady ? "核对我的 W3 理解地图" : "继续 W3 RAG 方案");
    setText("[data-copy-formal-entry]", "复制 W3 RAG 方案接力");

    setText('[data-later-stage-status="foundation"]', "W1 产物与两周突击线记录可作为输入");
    setText('[data-later-stage-status="week01"]', "作为长期线基础输入，不在本页改写");
    setText('[data-later-stage-status="week02"]', "作为 W3 选型前置输入，不在本页改写");
    setText('[data-later-stage-status="formal"]', "正式状态源未由本页改写");

    window.__STUDY_PAGE_HANDOFF__ = {
      track: "twelve_week_depth",
      week_id: config.week_id || "week-03",
      focus: config.day_label + " / " + config.knowledge_anchor,
      page_anchor: config.page_anchor,
      current_formal_state: formalProgress,
      next_surface: "Codex · 是否记录 Week 03 课程进程或启动正式评测",
      switch_condition: condition,
      formal_scope: "本页不创建 session、pending question、评分、复测关系或掌握证据；只生成 W3 RAG 知识库方案草稿。",
      reteach_contract: "若 W3 方案只停留在“接入知识库”，请按‘用户问题 → 资料源 → 切片索引 → 检索重排 → 引用更新 → 降级边界’重新收敛。"
    };
  }
  ...
  function setupFinalChallenge() {
    ...
    form.addEventListener("submit", function (event) {
      ...
      if (error) error.textContent = completed ? "" : "请沿五步路线补充 W3 RAG 知识库方案，并完成五项覆盖核对。";
      if (feedback) feedback.hidden = !completed;
      refreshProgressAndHandoff();
      saveState(completed ? "W3 综合 RAG 方案已保存；可以查看示例。" : "W3 RAG 方案还没有覆盖完整链路。");
    });
  }
  ...
  function answerCardMarkdown() {
    ...
    var lines = ["# 我的 W3 RAG 知识系统理解地图", ""];
    moduleOrder.forEach(function (id) {
      lines.push("## " + (moduleLabels[id] || id), answer(id + "_teachback", "还没有记录这一模块的学习笔记"), "");
    });
    lines.push("## W3 综合 RAG 方案", answer("final_diagnosis", "还没有完成 W3 综合工作坊"), "");
  ...
  function setupAnswerCard() {
    ...
    if (build) build.addEventListener("click", function () {
      ...
      saveState("W3 理解地图已由你的笔记和 RAG 方案汇总。");
    });
  ...
  function setupActions() {
    ...
    document.querySelectorAll("[data-copy-formal-entry]").forEach(function (button) {
      button.addEventListener("click", function () {
        copyText(JSON.stringify(window.__STUDY_PAGE_HANDOFF__ || {}, null, 2), "W3 RAG 方案接力已复制。");
      });
    });
  }
  ...
  if (state.migrated_legacy_key) saveState("旧 W3 草稿已迁移到新的学习闭环；原数据未删除。");
})();
