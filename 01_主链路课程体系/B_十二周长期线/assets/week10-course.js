(function () {
  ...
  function applyFormalState(stateSource) {
    ...
    var status = "W10 是十二周长期线的 Web Demo、API 安全代理与 RAG 接入页；当前正式主线仍以 study-project.json 为准。";
    ...
    var condition = practiceReady
      ? "W10 页面四项学习活动已完成；下一步是在 Codex 中确认是否进入正式评测或继续长期线。"
      : "先完成 W10 的六个工程机制模块与综合 Web Demo 工作坊；页面活动不会写入正式 JSON。";

    setText("[data-later-page-status]", status);
    setText("[data-later-cockpit-title]", "W10 工程演示：让 Demo 切片、前端状态、安全代理、RAG、校验恢复和部署观测支撑可演示产品判断。");
    setText("[data-later-cockpit-summary]", formalProgress + "网页只记录当前标签页草稿，不创建长期线正式掌握。");
    setText("[data-later-cockpit-mode]", "WEEK 10 PREVIEW");
    ...
    setText("[data-later-current-gap]", "W10 需要把独立 Web Demo、前后端状态、API 安全代理、RAG 接入、结构校验、错误恢复和演示部署放进同一条工程链。");
    setText("[data-later-formal-task]", "制定 W10 Web Demo、API 安全代理与 RAG 接入方案草稿；不写入正式状态源");
    setText("[data-later-current-surface]", "网页 · 十二周长期线 W10");
    setText("[data-later-switch-condition]", condition);
    setText("[data-later-sticky-surface]", "网页 · W10 Web Demo 与 RAG 接入");
    setText("[data-later-sticky-condition]", condition);
    setText("[data-later-formal-summary]", formalProgress);
    setText("[data-formal-handoff-title]", "W10 接力：页面完成后，再决定是否启动正式评测。");
    setText("[data-formal-handoff-summary]", "网页里的 Demo 切片、前后端边界、安全代理、RAG、校验恢复、部署观测和理解地图只是草稿证据。正式长期线是否记录 Week 10，需要 Codex 重新读取并写入唯一正式状态源。");
    setText("[data-formal-handoff-primary]", practiceReady ? "核对我的 W10 理解地图" : "继续 W10 工程方案");
    setText("[data-copy-formal-entry]", "复制 W10 Web Demo 接力");

    setText('[data-later-stage-status="foundation"]', "W1 产物与两周突击线记录可作为输入");
    setText('[data-later-stage-status="week01"]', "作为长期线基础输入，不在本页改写");
    setText('[data-later-stage-status="week02"]', "作为 W10 工程演示与 RAG 接入前置输入，不在本页改写");
    setText('[data-later-stage-status="formal"]', "正式状态源未由本页改写");

    window.__STUDY_PAGE_HANDOFF__ = {
      track: "twelve_week_depth",
      week_id: config.week_id || "week-10",
      focus: config.day_label + " / " + config.knowledge_anchor,
      page_anchor: config.page_anchor,
      current_formal_state: formalProgress,
      next_surface: "Codex · 是否记录 Week 10 课程进程或启动正式评测",
      switch_condition: condition,
      formal_scope: "本页不创建 session、pending question、评分、错题、复测关系、真实部署、API Key、向量索引、线上日志或掌握证据；只生成 W10 Web Demo 与 RAG 接入方案草稿。",
      reteach_contract: "若 W10 方案只停留在“页面能打开”，请按‘Demo Slice → Frontend State → Server Proxy → RAG → Validation/Recovery → Deployment Observability’重新收敛。"
    };
  }
  ...
  function setupFinalChallenge() {
    ...
    form.addEventListener("submit", function (event) {
      ...
      if (error) error.textContent = completed ? "" : "请沿五步路线补充 W10 Web Demo 工程方案，并完成五项覆盖核对。";
      if (feedback) feedback.hidden = !completed;
      refreshProgressAndHandoff();
      saveState(completed ? "W10 综合 Web Demo 工程方案已保存；可以查看示例。" : "W10 Web Demo 工程方案还没有覆盖完整链路。");
    });
  }
  ...
  function answerCardMarkdown() {
    ...
    var lines = ["# 我的 W10 Web Demo、API 安全代理与 RAG 接入理解地图", ""];
    moduleOrder.forEach(function (id) {
      lines.push("## " + (moduleLabels[id] || id), answer(id + "_teachback", "还没有记录这一模块的学习笔记"), "");
    });
    lines.push("## W10 综合 Web Demo 工程方案", answer("final_diagnosis", "还没有完成 W10 综合工作坊"), "");
  ...
  function setupAnswerCard() {
    ...
    if (build) build.addEventListener("click", function () {
      ...
      saveState("W10 理解地图已由你的笔记和工程方案草稿汇总。");
    });
  ...
  function setupActions() {
    ...
    document.querySelectorAll("[data-copy-formal-entry]").forEach(function (button) {
      button.addEventListener("click", function () {
        copyText(JSON.stringify(window.__STUDY_PAGE_HANDOFF__ || {}, null, 2), "W10 Web Demo 接力已复制。");
      });
    });
  }
  ...
  if (state.migrated_legacy_key) saveState("旧 W10 草稿已迁移到新的学习闭环；原数据未删除。");
})();
