(function () {
  ...
  function applyFormalState(stateSource) {
    ...
    var status = "W1 是十二周长期线的课程制定与承接页；当前正式主线仍以 study-project.json 为准。";
    ...
    var condition = practiceReady
      ? "W1 页面四项学习活动已完成；下一步是在 Codex 中确认是否把 W1 纳入正式长期线状态源。"
      : "先完成 W1 的五个模块与综合课程制定工作坊；页面活动不会写入正式 JSON。";

    setText("[data-later-page-status]", status);
    setText("[data-later-cockpit-title]", "W1 长期课程制定：先把十二周路线变成可执行的一周计划。");
    ...
    setText("[data-later-current-gap]", "W1 需要把旧知识、第 20 课、平台 Agent、证据看板和十二周节奏接成一条可执行计划。");
    setText("[data-later-formal-task]", "制定 W1 学习页草稿；不写入正式状态源");
    setText("[data-later-current-surface]", "网页 · 十二周长期线 W1");
    setText("[data-later-switch-condition]", condition);
    setText("[data-later-sticky-surface]", "网页 · W1 课程制定");
    setText("[data-later-sticky-condition]", condition);
    setText("[data-later-formal-summary]", formalProgress);
    setText("[data-formal-handoff-title]", "W1 接力：页面完成后，再决定是否启动长期线正式状态。");
    setText("[data-formal-handoff-summary]", "网页里的课程制定、练习、笔记和理解地图只是草稿证据。正式长期线是否开启、如何记录 Week 01，需要 Codex 重新读取并写入唯一正式状态源。");
    setText("[data-formal-handoff-primary]", practiceReady ? "核对我的 W1 理解地图" : "继续 W1 课程制定");
    setText("[data-copy-formal-entry]", "复制 W1 课程制定接力");
    ...
    window.__STUDY_PAGE_HANDOFF__ = {
      ...
      formal_scope: "本页不创建 session、pending question、评分、复测关系或掌握证据；只生成 W1 课程制定草稿。",
      reteach_contract: "若 W1 计划只停留在主题罗列，请按‘旧知识证据 → 未闭环缺口 → 本周动作 → 可验收产物 → 下一周依赖’重新收敛。"
    };
  }
  ...
  function setupFinalChallenge() {
    ...
    form.addEventListener("submit", function (event) {
      ...
      if (error) error.textContent = completed ? "" : "请沿五步路线补充 W1 计划，并完成五项覆盖核对。";
      if (feedback) feedback.hidden = !completed;
      refreshProgressAndHandoff();
      saveState(completed ? "W1 综合制定稿已保存；可以查看示例。" : "W1 制定稿还没有覆盖完整链路。");
    });
  }
  ...
  function answerCardMarkdown() {
    ...
    var lines = ["# 我的 W1 课程制定理解地图", ""];
    moduleOrder.forEach(function (id) {
      lines.push("## " + (moduleLabels[id] || id), answer(id + "_teachback", "还没有记录这一模块的学习笔记"), "");
    });
    lines.push("## W1 综合制定稿", answer("final_diagnosis", "还没有完成 W1 综合工作坊"), "");
  ...
  function setupActions() {
    ...
    document.querySelectorAll("[data-copy-formal-entry]").forEach(function (button) {
      button.addEventListener("click", function () {
        copyText(JSON.stringify(window.__STUDY_PAGE_HANDOFF__ || {}, null, 2), "W1 课程制定接力已复制。");
      });
    });
  }
  ...
  if (state.migrated_legacy_key) saveState("旧 W1 草稿已迁移到新的学习闭环；原数据未删除。");
})();
