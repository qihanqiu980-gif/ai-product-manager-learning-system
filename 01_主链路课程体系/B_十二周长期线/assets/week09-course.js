(function () {
  ...
  function applyFormalState(stateSource) {
    ...
    var status = "W9 是十二周长期线的 AI 商业指标与单位经济页；当前正式主线仍以 study-project.json 为准。";
    ...
    var condition = practiceReady
      ? "W9 页面四项学习活动已完成；下一步是在 Codex 中确认是否进入正式评测或继续长期线。"
      : "先完成 W9 的六个商业机制模块与综合商业闭环工作坊；页面活动不会写入正式 JSON。";

    setText("[data-later-page-status]", status);
    setText("[data-later-cockpit-title]", "W9 商业闭环：让激活、留存、付费、成本、路由和实验支撑单位经济判断。");
    setText("[data-later-cockpit-summary]", formalProgress + "网页只记录当前标签页草稿，不创建长期线正式掌握。");
    setText("[data-later-cockpit-mode]", "WEEK 09 PREVIEW");
    ...
    setText("[data-later-current-gap]", "W9 需要把激活、留存、付费、Token 成本、模型路由和商业实验放进同一条单位经济链。");
    setText("[data-later-formal-task]", "制定 W9 AI 商业指标与单位经济方案草稿；不写入正式状态源");
    setText("[data-later-current-surface]", "网页 · 十二周长期线 W9");
    setText("[data-later-switch-condition]", condition);
    setText("[data-later-sticky-surface]", "网页 · W9 AI 商业指标与单位经济");
    setText("[data-later-sticky-condition]", condition);
    setText("[data-later-formal-summary]", formalProgress);
    setText("[data-formal-handoff-title]", "W9 接力：页面完成后，再决定是否启动正式评测。");
    setText("[data-formal-handoff-summary]", "网页里的激活、留存、付费、成本、路由、实验方案和理解地图只是草稿证据。正式长期线是否记录 Week 09，需要 Codex 重新读取并写入唯一正式状态源。");
    setText("[data-formal-handoff-primary]", practiceReady ? "核对我的 W9 理解地图" : "继续 W9 商业闭环方案");
    setText("[data-copy-formal-entry]", "复制 W9 商业闭环接力");

    setText('[data-later-stage-status="foundation"]', "W1 产物与两周突击线记录可作为输入");
    setText('[data-later-stage-status="week01"]', "作为长期线基础输入，不在本页改写");
    setText('[data-later-stage-status="week02"]', "作为 W9 商业闭环与质量治理前置输入，不在本页改写");
    setText('[data-later-stage-status="formal"]', "正式状态源未由本页改写");

    window.__STUDY_PAGE_HANDOFF__ = {
      track: "twelve_week_depth",
      week_id: config.week_id || "week-09",
      focus: config.day_label + " / " + config.knowledge_anchor,
      page_anchor: config.page_anchor,
      current_formal_state: formalProgress,
      next_surface: "Codex · 是否记录 Week 09 课程进程或启动正式评测",
      switch_condition: condition,
      formal_scope: "本页不创建 session、pending question、评分、错题、复测关系、真实营收、成本、实验结论或掌握证据；只生成 W9 商业指标与单位经济方案草稿。",
      reteach_contract: "若 W9 方案只停留在“看转化率”，请按‘Activation → Retention → Payment → Unit Cost → Model Routing → Commercial Experiment’重新收敛。"
    };
  }
  ...
  function setupFinalChallenge() {
    ...
    form.addEventListener("submit", function (event) {
      ...
      if (error) error.textContent = completed ? "" : "请沿五步路线补充 W9 商业闭环方案，并完成五项覆盖核对。";
      if (feedback) feedback.hidden = !completed;
      refreshProgressAndHandoff();
      saveState(completed ? "W9 综合商业闭环方案已保存；可以查看示例。" : "W9 商业闭环方案还没有覆盖完整链路。");
    });
  }
  ...
  function answerCardMarkdown() {
    ...
    var lines = ["# 我的 W9 AI 商业指标与单位经济理解地图", ""];
    moduleOrder.forEach(function (id) {
      lines.push("## " + (moduleLabels[id] || id), answer(id + "_teachback", "还没有记录这一模块的学习笔记"), "");
    });
    lines.push("## W9 综合商业闭环方案", answer("final_diagnosis", "还没有完成 W9 综合工作坊"), "");
  ...
  function setupAnswerCard() {
    ...
    if (build) build.addEventListener("click", function () {
      ...
      saveState("W9 理解地图已由你的笔记和商业闭环方案草稿汇总。");
    });
  ...
  function setupActions() {
    ...
    document.querySelectorAll("[data-copy-formal-entry]").forEach(function (button) {
      button.addEventListener("click", function () {
        copyText(JSON.stringify(window.__STUDY_PAGE_HANDOFF__ || {}, null, 2), "W9 商业闭环接力已复制。");
      });
    });
  }
  ...
  if (state.migrated_legacy_key) saveState("旧 W9 草稿已迁移到新的学习闭环；原数据未删除。");
})();
