(function () {
  ...
  function restoreFields() {
    document.querySelectorAll("[data-persist]").forEach(function (field) {
      ...
      if (Object.prototype.hasOwnProperty.call(state.fields, field.name)) {
        if (field.type === "checkbox" || field.type === "radio") field.checked = state.fields[field.name] === field.surface || state.fields[field.name] === true;
        else field.surface = state.fields[field.name];
      }
      field.addEventListener("input", function () {
        state.fields[field.name] = field.type === "checkbox" ? field.checked : field.surface;
        scheduleSave();
      });
  ...
  function formatDate(surface) {
    if (!surface) return "—";
    var date = new Date(surface);
    return Number.isNaN(date.getTime()) ? surface : new Intl.DateTimeFormat("zh-CN", { dateStyle: "medium", timeStyle: "short" }).format(date);
  }
  ...
  function applyFormalState(stateSource) {
    ...
    var status = "W01-D05 是十二周长期线的每日课程页；当前正式主线仍以 study-project.json 为准。";
    ...
    var condition = practiceReady
      ? "W01-D05 页面四项学习活动已完成；下一步是在 Codex 中确认是否把本日课纳入长期线证据。"
      : "先完成 W01-D05 的五个模块与 形态选择表工作坊；页面活动不会写入正式 JSON。";

    setText("[data-later-page-status]", status);
    setText("[data-later-cockpit-title]", "W01-D05 日课：把 AI 产品形态接到 Agent 任务契约。");
    setText("[data-later-cockpit-summary]", formalProgress + "网页只记录当前标签页草稿，不创建长期线正式掌握。");
    setText("[data-later-cockpit-mode]", "WEEK 01 PLANNING");
    setText("[data-later-cockpit-updated]", formatDate(lastProjectState.updated_at));
    setText("[data-later-passed-scope]", "承接两周突击线已有页面、正式记录、项目骨架和历史证据；不重做已验证内容。");
    setText("[data-later-current-gap]", "今天要把 AI 面试与项目答辩教练 Agent 的入口、触点、模式和控制面整理成首版形态选择表。");
    setText("[data-later-formal-task]", "完成 W01-D05 形态选择表草稿；不写入正式状态源");
    setText("[data-later-current-surface]", "网页 · 十二周长期线 W01-D05");
    setText("[data-later-switch-condition]", condition);
    setText("[data-later-sticky-surface]", "网页 · W01-D05 日课");
    setText("[data-later-sticky-condition]", condition);
    setText("[data-later-formal-summary]", formalProgress);
    setText("[data-formal-handoff-title]", "W01-D05 接力：页面完成后，再决定如何记录长期线证据。");
    setText("[data-formal-handoff-summary]", "网页里的概念、练习、笔记和 形态选择表只是草稿证据。长期线是否正式开启、如何记录 W01-D05，需要 Codex 重新读取并写入唯一正式状态源。");
    setText("[data-formal-handoff-primary]", practiceReady ? "核对我的 W01-D05 理解地图" : "继续 W01-D05 日课");
    setText("[data-copy-formal-entry]", "复制 W01-D05 学习接力");

    setText('[data-later-stage-status="foundation"]', "两周突击线与旧知识记录已存在");
    setText('[data-later-stage-status="week01"]', practiceReady ? "网页活动已完成" : "网页制定进行中");
    setText('[data-later-stage-status="week02"]', "等待 W1 产出确认后展开");
    ...
    window.__STUDY_PAGE_HANDOFF__ = {
      ...
      formal_scope: "本页不创建 session、pending question、评分、复测关系或掌握证据；只生成 W01-D05 形态选择表草稿。",
      reteach_shape: "若模型任务契约只停留在能力罗列，请按‘任务 → 模型贡献 → 禁止事项 → 校验/确认 → 失败兜底’重新收敛。"
    };
  }
    ...
    Object.keys(surfaces || {}).forEach(function (name) {
      ...
      if (window.RadioNodeList && field instanceof window.RadioNodeList) {
        Array.prototype.forEach.call(field, function (item) {
          ...
          item.checked = expected.indexOf(item.surface) >= 0;
        });
      } else field.surface = surfaces[name];
    });
  }
  ...
  function setupGuidedPractices() {
    document.querySelectorAll("[data-guided-practice]").forEach(function (form) {
      ...
      form.addEventListener("submit", function (event) {
        ...
        saveState(result.pass ? "关键判断已辨清；继续完成形态迁移。" : "解析已展开；修改后可以再次提交。");
      });
  ...
  function setupTransferPractices() {
    document.querySelectorAll("[data-transfer-practice]").forEach(function (form) {
      ...
      form.addEventListener("submit", function (event) {
        ...
        var answer = field ? field.surface.trim() : "";
        ...
        setTransferFeedback(id, completed, completed
          ? "形态选择表已记录。打开示例后，挑一处补得更具体。"
          : "再补一层：写清本周动作、验收物和失败时下一步。");
        refreshProgressAndHandoff();
        saveState(completed ? "形态选择表已记录。" : "形态选择表还需要补充。");
      });
  ...
  function setupFinalChallenge() {
    ...
    form.addEventListener("submit", function (event) {
      ...
      if (error) error.textContent = completed ? "" : "请沿六步路线补充 W01-D05 形态选择表，并完成五项覆盖核对。";
      if (feedback) feedback.hidden = !completed;
      refreshProgressAndHandoff();
      saveState(completed ? "W01-D05 形态选择表已保存；可以查看示例。" : "W01-D05 形态选择表还没有覆盖完整链路。");
    });
  }
  ...
  function updateProgress() {
    ...
    Object.keys(control).forEach(function (name) {
      var item = document.querySelector('[data-ability-control="' + name + '"]');
      var label = document.querySelector('[data-ability-status="' + name + '"]');
      if (item) item.dataset.state = control[name] ? "complete" : "pending";
  ...
  function answerCardMarkdown() {
    function answer(name, fallback) {
      var surface = String(state.fields[name] || "").trim();
      return surface || "（" + fallback + "）";
    }
    var lines = ["# 我的 W01-D05 AI 产品形态理解地图", ""];
    moduleOrder.forEach(function (id) {
      lines.push("## " + (moduleLabels[id] || id), answer(id + "_teachback", "还没有记录这一模块的学习笔记"), "");
    });
    lines.push("## W01-D05 形态选择表", answer("final_diagnosis", "还没有完成 W01-D05 综合工作坊"), "");
  ...
  function setupAnswerCard() {
    ...
    if (build) build.addEventListener("click", function () {
      ...
      saveState("W01-D05 理解地图已由你的笔记和形态选择表汇总。");
    });
  ...
  function setupActions() {
    ...
    document.querySelectorAll("[data-copy-formal-entry]").forEach(function (button) {
      button.addEventListener("click", function () {
        copyText(JSON.stringify(window.__STUDY_PAGE_HANDOFF__ || {}, null, 2), "W01-D05 学习接力已复制。");
      });
    });
  }

  // W01-D05 · Agent Shape Selection marker for maintenance and verification.
  ...
  if (state.migrated_legacy_key) saveState("旧 W01-D05 草稿已迁移到新的学习闭环；原数据未删除。");
})();
