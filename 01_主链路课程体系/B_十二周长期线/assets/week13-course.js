(function () {
  ...
  function applyFormalState(stateSource) {
    ...
    var status = "W13 是十二周长期线之后的反馈迭代与能力补证延展页；当前正式主线仍以 study-project.json 为准。";
    ...
    var condition = practiceReady
      ? "W13 页面四项学习活动已完成；下一步是在 Codex 中确认是否记录延展维护进度或安排正式补证。"
      : "先完成 W13 的六个反馈迭代模块与综合维护计划工作坊；页面活动不会写入正式 JSON。";

    setText("[data-later-page-status]", status);
    setText("[data-later-cockpit-title]", "W13 反馈迭代：让投递信号、缺口排序、作品集版本、投递实验、补证卡和周节奏持续运转。");
    setText("[data-later-cockpit-summary]", formalProgress + "网页只记录当前标签页草稿，不创建长期线正式掌握。");
    setText("[data-later-cockpit-mode]", "WEEK 13 EXTENSION");
    setText("[data-later-cockpit-updated]", formatDate(lastProjectState.updated_at));
    setText("[data-later-passed-scope]", "承接 W12 的作品集、岗位映射与答辩方案；不重写十二周主线结论。");
    setText("[data-later-current-gap]", "W13 需要把真实反馈、模拟反馈、作品集改版、投递实验和能力补证放进同一条维护链。");
    setText("[data-later-formal-task]", "制定 W13 反馈迭代与能力补证草稿；不写入正式状态源");
    setText("[data-later-current-surface]", "网页 · 十二周长期线延展 W13");
    setText("[data-later-switch-condition]", condition);
    setText("[data-later-sticky-surface]", "网页 · W13 反馈迭代与能力补证");
    setText("[data-later-sticky-condition]", condition);
    setText("[data-later-formal-summary]", formalProgress);
    setText("[data-formal-handoff-title]", "W13 接力：页面完成后，再决定是否记录延展维护进度。");
    setText("[data-formal-handoff-summary]", "网页里的反馈记录模板、缺口排序、作品集版本、投递实验、补证卡、周节奏和理解地图只是草稿证据。正式状态是否记录 Week 13 延展维护，需要 Codex 重新读取并写入唯一正式状态源。");
    setText("[data-formal-handoff-primary]", practiceReady ? "核对我的 W13 理解地图" : "继续 W13 迭代计划");
    setText("[data-copy-formal-entry]", "复制 W13 反馈迭代接力");

    setText('[data-later-stage-status="foundation"]', "两周突击线与 W1-W12 产物可作为输入");
    setText('[data-later-stage-status="week12"]', "作为 W13 延展维护前置输入，不在本页改写");
    setText('[data-later-stage-status="evidence"]', "真实、模拟与草稿反馈需要分层标注");
    setText('[data-later-stage-status="formal"]', "正式状态源未由本页改写");

    window.__STUDY_PAGE_HANDOFF__ = {
      track: "twelve_week_depth_extension",
      week_id: config.week_id || "week-13",
      focus: config.day_label + " / " + config.knowledge_anchor,
      page_anchor: config.page_anchor,
      current_formal_state: formalProgress,
      next_surface: "Codex · 是否记录 Week 13 延展维护进度或启动正式补证",
      switch_condition: condition,
      formal_scope: "本页不创建 session、pending question、评分、错题、复测关系、真实投递、真实面试、offer、岗位背书、商业成果或掌握证据；只生成 W13 反馈迭代与能力补证草稿。",
      reteach_contract: "若 W13 计划只停留在“继续投递和继续改作品集”，请按‘Signal Log → Gap Triage → Portfolio Version → Application Experiment → Remediation Card → Weekly Cadence’重新收敛。"
    };
  }
  ...
  function setupFinalChallenge() {
    ...
    form.addEventListener("submit", function (event) {
      ...
      if (error) error.textContent = completed ? "" : "请沿五步路线补充 W13 反馈迭代计划，并完成五项覆盖核对。";
      if (feedback) feedback.hidden = !completed;
      refreshProgressAndHandoff();
      saveState(completed ? "W13 综合反馈迭代计划已保存；可以查看示例。" : "W13 反馈迭代计划还没有覆盖完整链路。");
    });
  }
  ...
  function answerCardMarkdown() {
    ...
    var lines = ["# 我的 W13 反馈迭代与能力补证理解地图", ""];
    moduleOrder.forEach(function (id) {
      lines.push("## " + (moduleLabels[id] || id), answer(id + "_teachback", "还没有记录这一模块的学习笔记"), "");
    });
    lines.push("## W13 综合反馈迭代计划", answer("final_diagnosis", "还没有完成 W13 综合工作坊"), "");
    lines.push("> 这是网页学习草稿，不是真实投递、面试、offer 或正式掌握证据；真实反馈、模拟反馈和网页草稿应分层标注。");
    return lines.join("\n");
  }
  ...
  function setupAnswerCard() {
    ...
    if (build) build.addEventListener("click", function () {
      ...
      saveState("W13 理解地图已由你的笔记和迭代计划草稿汇总。");
    });
  ...
  function setupActions() {
    ...
    document.querySelectorAll("[data-copy-formal-entry]").forEach(function (button) {
      button.addEventListener("click", function () {
        copyText(JSON.stringify(window.__STUDY_PAGE_HANDOFF__ || {}, null, 2), "W13 反馈迭代接力已复制。");
      });
    });
  }
  ...
  if (state.migrated_legacy_key) saveState("旧 W13 草稿已迁移到新的学习闭环；原数据未删除。");
})();
