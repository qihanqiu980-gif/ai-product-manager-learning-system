(function () {
  ...
  function applyFormalState(stateSource) {
    ...
    var status = "W11 是十二周长期线的 Demo 稳定化、移动端无障碍与部署回归页；当前正式主线仍以 study-project.json 为准。";
    ...
    var condition = practiceReady
      ? "W11 页面四项学习活动已完成；下一步是在 Codex 中确认是否进入正式评测或继续长期线。"
      : "先完成 W11 的六个稳定化机制模块与综合发布回归工作坊；页面活动不会写入正式 JSON。";

    setText("[data-later-page-status]", status);
    setText("[data-later-cockpit-title]", "W11 稳定演示：让功能范围、边界状态、响应时间、成本护栏、移动端无障碍和部署回归支撑可评测版本判断。");
    setText("[data-later-cockpit-summary]", formalProgress + "网页只记录当前标签页草稿，不创建长期线正式掌握。");
    setText("[data-later-cockpit-mode]", "WEEK 11 PREVIEW");
    ...
    setText("[data-later-current-gap]", "W11 需要把功能范围、边界状态、响应时间、成本护栏、移动端、无障碍、部署和回归放进同一条稳定化链。");
    setText("[data-later-formal-task]", "制定 W11 Demo 稳定化、移动端无障碍与部署回归方案草稿；不写入正式状态源");
    setText("[data-later-current-surface]", "网页 · 十二周长期线 W11");
    setText("[data-later-switch-condition]", condition);
    setText("[data-later-sticky-surface]", "网页 · W11 Demo 稳定化与部署回归");
    setText("[data-later-sticky-condition]", condition);
    setText("[data-later-formal-summary]", formalProgress);
    setText("[data-formal-handoff-title]", "W11 接力：页面完成后，再决定是否启动正式评测。");
    setText("[data-formal-handoff-summary]", "网页里的功能范围、边界状态、响应时间、成本护栏、移动端无障碍、部署回归和理解地图只是草稿证据。正式长期线是否记录 Week 11，需要 Codex 重新读取并写入唯一正式状态源。");
    setText("[data-formal-handoff-primary]", practiceReady ? "核对我的 W11 理解地图" : "继续 W11 稳定化方案");
    setText("[data-copy-formal-entry]", "复制 W11 稳定化接力");

    setText('[data-later-stage-status="foundation"]', "W1 产物与两周突击线记录可作为输入");
    setText('[data-later-stage-status="week01"]', "作为长期线基础输入，不在本页改写");
    setText('[data-later-stage-status="week02"]', "作为 W11 稳定化与部署回归前置输入，不在本页改写");
    setText('[data-later-stage-status="formal"]', "正式状态源未由本页改写");

    window.__STUDY_PAGE_HANDOFF__ = {
      track: "twelve_week_depth",
      week_id: config.week_id || "week-11",
      focus: config.day_label + " / " + config.knowledge_anchor,
      page_anchor: config.page_anchor,
      current_formal_state: formalProgress,
      next_surface: "Codex · 是否记录 Week 11 课程进程或启动正式评测",
      switch_condition: condition,
      formal_scope: "本页不创建 session、pending question、评分、错题、复测关系、真实部署、真实监控、成本账单、线上回归记录或掌握证据；只生成 W11 Demo 稳定化与部署回归方案草稿。",
      reteach_contract: "若 W11 方案只停留在“部署成功”，请按‘Feature Scope → Boundary States → Latency Budget → Cost Guardrails → Mobile/A11y → Release Regression’重新收敛。"
    };
  }
  ...
  function setupFinalChallenge() {
    ...
    form.addEventListener("submit", function (event) {
      ...
      if (error) error.textContent = completed ? "" : "请沿五步路线补充 W11 Demo 稳定化方案，并完成五项覆盖核对。";
      if (feedback) feedback.hidden = !completed;
      refreshProgressAndHandoff();
      saveState(completed ? "W11 综合稳定化方案已保存；可以查看示例。" : "W11 稳定化方案还没有覆盖完整链路。");
    });
  }
  ...
  function answerCardMarkdown() {
    ...
    var lines = ["# 我的 W11 Demo 稳定化、移动端无障碍与部署回归理解地图", ""];
    moduleOrder.forEach(function (id) {
      lines.push("## " + (moduleLabels[id] || id), answer(id + "_teachback", "还没有记录这一模块的学习笔记"), "");
    });
    lines.push("## W11 综合 Demo 稳定化方案", answer("final_diagnosis", "还没有完成 W11 综合工作坊"), "");
  ...
  function setupAnswerCard() {
    ...
    if (build) build.addEventListener("click", function () {
      ...
      saveState("W11 理解地图已由你的笔记和稳定化方案草稿汇总。");
    });
  ...
  function setupActions() {
    ...
    document.querySelectorAll("[data-copy-formal-entry]").forEach(function (button) {
      button.addEventListener("click", function () {
        copyText(JSON.stringify(window.__STUDY_PAGE_HANDOFF__ || {}, null, 2), "W11 稳定化接力已复制。");
      });
    });
  }
  ...
  if (state.migrated_legacy_key) saveState("旧 W11 草稿已迁移到新的学习闭环；原数据未删除。");
})();
