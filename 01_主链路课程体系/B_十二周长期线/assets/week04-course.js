(function () {
  ...
  function applyFormalState(stateSource) {
    ...
    var status = "W4 是十二周长期线的自适应评测流程页；当前正式主线仍以 study-project.json 为准。";
    ...
    var condition = practiceReady
      ? "W4 页面四项学习活动已完成；下一步是在 Codex 中确认是否进入正式评测或继续长期线。"
      : "先完成 W4 的六个自适应评测机制模块与综合面试流程工作坊；页面活动不会写入正式 JSON。";

    setText("[data-later-page-status]", status);
    setText("[data-later-cockpit-title]", "W4 自适应评测：让题目、追问、状态和停止条件由规则守住。");
    setText("[data-later-cockpit-summary]", formalProgress + "网页只记录当前标签页草稿，不创建长期线正式掌握。");
    setText("[data-later-cockpit-mode]", "WEEK 04 PREVIEW");
    setText("[data-later-cockpit-updated]", formatDate(lastProjectState.updated_at));
    setText("[data-later-passed-scope]", "承接 W1-W3 的机制、知识库与证据边界；不重做已验证内容。");
    setText("[data-later-current-gap]", "W4 需要把能力覆盖、题目选择、追问、上下文、状态机和停止规则放进同一套可解释的自适应评测流程。");
    setText("[data-later-formal-task]", "制定 W4 自适应面试流程草稿；不写入正式状态源");
    setText("[data-later-current-surface]", "网页 · 十二周长期线 W4");
    setText("[data-later-switch-condition]", condition);
    setText("[data-later-sticky-surface]", "网页 · W4 自适应评测流程");
    setText("[data-later-sticky-condition]", condition);
    setText("[data-later-formal-summary]", formalProgress);
    setText("[data-formal-handoff-title]", "W4 接力：页面完成后，再决定是否启动正式评测。");
    setText("[data-formal-handoff-summary]", "网页里的自适应评测判断、面试流程方案、笔记和理解地图只是草稿证据。正式长期线是否记录 Week 04，需要 Codex 重新读取并写入唯一正式状态源。");
    setText("[data-formal-handoff-primary]", practiceReady ? "核对我的 W4 理解地图" : "继续 W4 面试流程");
    setText("[data-copy-formal-entry]", "复制 W4 自适应评测接力");

    setText('[data-later-stage-status="foundation"]', "W1 产物与两周突击线记录可作为输入");
    setText('[data-later-stage-status="week01"]', "作为长期线基础输入，不在本页改写");
    setText('[data-later-stage-status="week02"]', "作为 W4 评测流程前置输入，不在本页改写");
    setText('[data-later-stage-status="formal"]', "正式状态源未由本页改写");

    window.__STUDY_PAGE_HANDOFF__ = {
      track: "twelve_week_depth",
      week_id: config.week_id || "week-04",
      focus: config.day_label + " / " + config.knowledge_anchor,
      page_anchor: config.page_anchor,
      current_formal_state: formalProgress,
      next_surface: "Codex · 是否记录 Week 04 课程进程或启动正式评测",
      switch_condition: condition,
      formal_scope: "本页不创建 session、pending question、评分、错题、复测关系或掌握证据；只生成 W4 自适应面试流程草稿。",
      reteach_contract: "若 W4 方案只停留在“让模型继续追问”，请按‘能力目标 → 覆盖缺口 → 题目选择 → 追问额度 → 状态机 → 停止条件’重新收敛。"
    };
  }
  ...
  function setupFinalChallenge() {
    ...
    form.addEventListener("submit", function (event) {
      ...
      if (error) error.textContent = completed ? "" : "请沿五步路线补充 W4 自适应面试流程，并完成五项覆盖核对。";
      if (feedback) feedback.hidden = !completed;
      refreshProgressAndHandoff();
      saveState(completed ? "W4 综合面试流程已保存；可以查看示例。" : "W4 面试流程还没有覆盖完整链路。");
    });
  }
  ...
  function answerCardMarkdown() {
    ...
    var lines = ["# 我的 W4 自适应评测理解地图", ""];
    moduleOrder.forEach(function (id) {
      lines.push("## " + (moduleLabels[id] || id), answer(id + "_teachback", "还没有记录这一模块的学习笔记"), "");
    });
    lines.push("## W4 综合面试流程", answer("final_diagnosis", "还没有完成 W4 综合工作坊"), "");
  ...
  function setupAnswerCard() {
    ...
    if (build) build.addEventListener("click", function () {
      ...
      saveState("W4 理解地图已由你的笔记和面试流程方案汇总。");
    });
  ...
  function setupActions() {
    ...
    document.querySelectorAll("[data-copy-formal-entry]").forEach(function (button) {
      button.addEventListener("click", function () {
        copyText(JSON.stringify(window.__STUDY_PAGE_HANDOFF__ || {}, null, 2), "W4 自适应评测接力已复制。");
      });
    });
  }
  ...
  if (state.migrated_legacy_key) saveState("旧 W4 草稿已迁移到新的学习闭环；原数据未删除。");
})();
