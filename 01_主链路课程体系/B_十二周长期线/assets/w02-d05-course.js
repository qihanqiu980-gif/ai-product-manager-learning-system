(function () {
  ...
  function applyFormalState(stateSource) {
    ...
    var status = "W02-D05 是十二周长期线的每日课程页；当前正式主线仍以 study-project.json 为准。";
    ...
    var condition = practiceReady
      ? "W02-D05 页面四项学习活动已完成；下一步是在 Codex 中确认是否把本日课纳入长期线证据。"
      : "先完成 W02-D05 的五个模块与工具调用判断表工作坊；页面活动不会写入正式 JSON。";

    setText("[data-later-page-status]", status);
    setText("[data-later-cockpit-title]", "W02-D05 日课：先把 数据与材料变成 Agent 技术选型输入。");
    ...
    setText("[data-later-formal-task]", "完成 W02-D05 材料清单草稿；不写入正式状态源");
    setText("[data-later-current-surface]", "网页 · 十二周长期线 W02-D05");
    setText("[data-later-switch-condition]", condition);
    setText("[data-later-sticky-surface]", "网页 · W02-D05 日课");
    setText("[data-later-sticky-condition]", condition);
    setText("[data-later-formal-summary]", formalProgress);
    setText("[data-formal-handoff-title]", "W02-D05 接力：页面完成后，再决定如何记录长期线证据。");
    setText("[data-formal-handoff-summary]", "网页里的概念、练习、笔记和材料清单只是草稿证据。长期线是否正式开启、如何记录 W02-D05，需要 Codex 重新读取并写入唯一正式状态源。");
    setText("[data-formal-handoff-primary]", practiceReady ? "核对我的 W02-D05 理解地图" : "继续 W02-D05 日课");
    setText("[data-copy-formal-entry]", "复制 W02-D05 学习接力");
    ...
    setText('[data-later-stage-status="next"]', "等待 W02-D05 边界表确认后展开");
    ...
    window.__STUDY_PAGE_HANDOFF__ = {
      ...
      formal_scope: "本页不创建 session、pending question、评分、复测关系或掌握证据；只生成 W02-D05 材料清单草稿。",
  ...
  function setupFinalChallenge() {
    ...
    form.addEventListener("submit", function (event) {
      ...
      if (error) error.textContent = completed ? "" : "请沿五步路线补充 W02-D05 材料清单，并完成五项覆盖核对。";
      if (feedback) feedback.hidden = !completed;
      refreshProgressAndHandoff();
      saveState(completed ? "W02-D05 材料清单已保存；可以查看示例。" : "W02-D05 缺口表还没有覆盖完整链路。");
    });
  }
  ...
  function answerCardMarkdown() {
    ...
    var lines = ["# 我的 W02-D05 Agent 工具调用理解地图", ""];
    moduleOrder.forEach(function (id) {
      lines.push("## " + (moduleLabels[id] || id), answer(id + "_teachback", "还没有记录这一模块的学习笔记"), "");
    });
    lines.push("## W02-D05 材料清单", answer("final_diagnosis", "还没有完成 W02-D05 综合工作坊"), "");
  ...
  function setupAnswerCard() {
    ...
    if (build) build.addEventListener("click", function () {
      ...
      saveState("W02-D05 理解地图已由你的笔记和材料清单汇总。");
    });
  ...
  function setupActions() {
    ...
    document.querySelectorAll("[data-copy-formal-entry]").forEach(function (button) {
      button.addEventListener("click", function () {
        copyText(JSON.stringify(window.__STUDY_PAGE_HANDOFF__ || {}, null, 2), "W02-D05 学习接力已复制。");
      });
    });
  }
  ...
  if (state.migrated_legacy_key) saveState("旧 W02-D05 草稿已迁移到新的学习闭环；原数据未删除。");
})();
