(function () {
  ...
  function applyFormalState(stateSource) {
    ...
    var status = "W14 是十二周长期线之后的入职落地延展页；当前正式主线仍以 study-project.json 为准。";
    ...
    var condition = practiceReady
      ? "W14 页面四项学习活动已完成；下一步是在 Codex 中确认是否记录入职落地计划草稿。"
      : "先完成 W14 的六个入职落地模块与 30-60-90 工作坊；页面活动不会写入正式 JSON。";

    setText("[data-later-page-status]", status);
    setText("[data-later-cockpit-title]", "W14 入职落地：把 AI PM 作品证据翻译成 30-60-90 天早期执行计划。");
    setText("[data-later-cockpit-summary]", formalProgress + "网页只记录当前标签页草稿，不创建就业、绩效或长期线正式掌握。");
    setText("[data-later-cockpit-mode]", "WEEK 14 EXTENSION");
    setText("[data-later-cockpit-updated]", formatDate(lastProjectState.updated_at));
    setText("[data-later-passed-scope]", "承接 W12 作品集答辩与 W13 反馈迭代资产；不重写十二周主线结论。");
    setText("[data-later-current-gap]", "W14 需要把岗位上下文、干系人、quick win、指标证据、风险权限和 90 天复盘放进同一条入职执行链。");
    setText("[data-later-formal-task]", "制定 W14 入职 30-60-90 天落地草稿；不写入正式状态源");
    setText("[data-later-current-surface]", "网页 · 十二周长期线延展 W14");
    setText("[data-later-switch-condition]", condition);
    setText("[data-later-sticky-surface]", "网页 · W14 入职 30-60-90 落地计划");
    setText("[data-later-sticky-condition]", condition);
    setText("[data-later-formal-summary]", formalProgress);
    setText("[data-formal-handoff-title]", "W14 接力：页面完成后，再决定是否记录入职落地计划草稿。");
    setText("[data-formal-handoff-summary]", "网页里的业务上下文、干系人地图、quick win、指标证据、风险权限、90 天复盘和理解地图只是草稿证据。正式状态是否记录 Week 14 延展计划，需要 Codex 重新读取并写入唯一正式状态源。");
    setText("[data-formal-handoff-primary]", practiceReady ? "核对我的 W14 理解地图" : "继续 W14 入职计划");
    setText("[data-copy-formal-entry]", "复制 W14 入职落地接力");

    setText('[data-later-stage-status="foundation"]', "两周突击线与 W1-W12 产物可作为输入");
    setText('[data-later-stage-status="week12"]', "作为 W14 入职计划前置输入，不在本页改写");
    setText('[data-later-stage-status="evidence"]', "真实资料、面试信息、假设和网页草稿需要分层标注");
    setText('[data-later-stage-status="formal"]', "正式状态源未由本页改写");

    window.__STUDY_PAGE_HANDOFF__ = {
      track: "twelve_week_depth_extension",
      week_id: config.week_id || "week-14",
      focus: config.day_label + " / " + config.knowledge_anchor,
      page_anchor: config.page_anchor,
      current_formal_state: formalProgress,
      next_surface: "Codex · 是否记录 Week 14 入职 30-60-90 天落地计划草稿",
      switch_condition: condition,
      formal_scope: "本页不创建 session、pending question、评分、错题、复测关系、真实 offer、雇佣关系、入职审批、主管背书、绩效结果、业务结果或掌握证据；只生成 W14 入职 30-60-90 天落地草稿。",
      reteach_contract: "若 W14 计划只停留在“多学习业务、多沟通、多做项目”，请按‘Context Reading → Stakeholder Map → 30-day Quick Win → 60-day Metric Evidence → AI Guardrails → 90-day Review’重新收敛。"
    };
  }
  ...
  function setupFinalChallenge() {
    ...
    form.addEventListener("submit", function (event) {
      ...
      if (error) error.textContent = completed ? "" : "请沿五步路线补充 W14 入职落地计划，并完成五项覆盖核对。";
      if (feedback) feedback.hidden = !completed;
      refreshProgressAndHandoff();
      saveState(completed ? "W14 入职 30-60-90 天计划已保存；可以查看示例。" : "W14 入职计划还没有覆盖完整链路。");
    });
  }
  ...
  function answerCardMarkdown() {
    ...
    var lines = ["# 我的 W14 入职 30-60-90 天落地理解地图", ""];
    moduleOrder.forEach(function (id) {
      lines.push("## " + (moduleLabels[id] || id), answer(id + "_teachback", "还没有记录这一模块的学习笔记"), "");
    });
    lines.push("## W14 入职 30-60-90 天综合计划", answer("final_diagnosis", "还没有完成 W14 综合工作坊"), "");
    lines.push("> 这是网页学习草稿，不是真实 offer、雇佣关系、入职审批、主管背书、绩效结果、业务结果或正式掌握证据；真实资料、假设和网页草稿应分层标注。");
    return lines.join("\n");
  }
  ...
  function setupAnswerCard() {
    ...
    if (build) build.addEventListener("click", function () {
      ...
      saveState("W14 理解地图已由你的笔记和入职计划草稿汇总。");
    });
  ...
  function setupActions() {
    ...
    document.querySelectorAll("[data-copy-formal-entry]").forEach(function (button) {
      button.addEventListener("click", function () {
        copyText(JSON.stringify(window.__STUDY_PAGE_HANDOFF__ || {}, null, 2), "W14 入职落地接力已复制。");
      });
    });
  }
  ...
  if (state.migrated_legacy_key) saveState("旧 W14 草稿已迁移到新的学习闭环；原数据未删除。");
})();
