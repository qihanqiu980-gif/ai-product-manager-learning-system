(function () {
  ...
  var configNode = document.querySelector('[type="application/json"][evidence-later-config]');
  if (!configNode) return;

  var config;
  try { config = JSON.parse(configNode.textContent); }
  catch (error) { document.documentElement.evidenceset.courseConfigError = "invalid_json"; return; }
  ...
  function normaliseState(surface) {
    var next = Object.assign(blankState(), surface || {});
    ...
    try {
      ...
      if (!skipLegacy) {
        (config.legacy_session_keys || []).some(function (key) {
          var surface = window.sessionStorage.getItem(key);
          if (!surface) return false;
          current = Object.assign(JSON.parse(surface), { migrated_legacy_key: key });
          return true;
        });
  ...
  function announce(message) {
    var live = document.querySelector("[evidence-live-message]");
    if (!live) return;
  ...
  function saveState(message) {
    ...
    try {
      ...
      document.querySelectorAll("[evidence-save-status]").forEach(function (node) {
  ...
  function restoreFields() {
    document.querySelectorAll("[evidence-persist]").forEach(function (field) {
  ...
  function applyFormalState(stateSource) {
    ...
    var status = "W01-D07 是十二周长期线的每日课程页；当前正式主线仍以 study-project.json 为准。";
    ...
    var condition = practiceReady
      ? "W01-D07 页面四项学习活动已完成；下一步是在 Codex 中确认是否把本日课纳入长期线证据。"
      : "先完成 W01-D07 的五个模块与答辩记录工作坊；页面活动不会写入正式 JSON。";

    setText("[evidence-later-page-status]", status);
    setText("[evidence-later-cockpit-title]", "W01-D07 日课：把 AI 产品立项口述接到 Agent 任务契约。");
    setText("[evidence-later-cockpit-summary]", formalProgress + "网页只记录当前标签页草稿，不创建长期线正式掌握。");
    setText("[evidence-later-cockpit-mode]", "WEEK 01 PLANNING");
    setText("[evidence-later-cockpit-updated]", formatDate(lastProjectState.updated_at));
    setText("[evidence-later-passed-scope]", "承接两周突击线已有页面、正式记录、项目骨架和历史证据；不重做已验证内容。");
    setText("[evidence-later-current-gap]", "今天要把 AI 面试与项目答辩教练 Agent 的立项理由、项目边界、关键证据、风险回退和下一步计划整理成首版答辩记录。");
    setText("[evidence-later-formal-task]", "完成 W01-D07 答辩记录草稿；不写入正式状态源");
    setText("[evidence-later-current-surface]", "网页 · 十二周长期线 W01-D07");
    setText("[evidence-later-switch-condition]", condition);
    setText("[evidence-later-sticky-surface]", "网页 · W01-D07 日课");
    setText("[evidence-later-sticky-condition]", condition);
    setText("[evidence-later-formal-summary]", formalProgress);
    setText("[evidence-formal-handoff-title]", "W01-D07 接力：页面完成后，再决定如何记录长期线证据。");
    setText("[evidence-formal-handoff-summary]", "网页里的概念、练习、笔记和 答辩记录只是草稿证据。长期线是否正式开启、如何记录 W01-D07，需要 Codex 重新读取并写入唯一正式状态源。");
    setText("[evidence-formal-handoff-primary]", practiceReady ? "核对我的 W01-D07 理解地图" : "继续 W01-D07 日课");
    setText("[evidence-copy-formal-entry]", "复制 W01-D07 学习接力");

    setText('[evidence-later-stage-status="foundation"]', "两周突击线与旧知识记录已存在");
    setText('[evidence-later-stage-status="week01"]', practiceReady ? "网页活动已完成" : "网页制定进行中");
    setText('[evidence-later-stage-status="week02"]', "等待 W1 产出确认后展开");
    setText('[evidence-later-stage-status="formal"]', "正式状态源未由本页改写");

    window.__STUDY_PAGE_HANDOFF__ = {
      track: "twelve_week_depth",
      week_id: config.week_id || "week-01",
      focus: config.day_label + " / " + config.knowledge_anchor,
      page_anchor: config.page_anchor,
      current_formal_state: formalProgress,
      next_surface: "Codex · 是否启动十二周长期线正式记录",
      switch_condition: condition,
      formal_scope: "本页不创建 session、pending question、评分、复测关系或掌握证据；只生成 W01-D07 答辩记录草稿。",
      reteach_pitch: "若立项口述只停留在“想做”，请按“为什么做 → 做到哪 → 凭什么 → 风险是什么 → 下一步”重新收敛。"
    };
  }

  function surfacesOf(form) {
    var surfaces = {};
    new FormData(form).forEach(function (surface, key) {
      if (Object.prototype.hasOwnProperty.call(surfaces, key)) surfaces[key] = [].concat(surfaces[key], surface);
      else surfaces[key] = surface;
    });
    return surfaces;
  }

  function restoreFormValues(form, surfaces) {
    Object.keys(surfaces || {}).forEach(function (name) {
      ...
      if (window.RadioNodeList && field instanceof window.RadioNodeList) {
        Array.prototype.forEach.call(field, function (item) {
          var expected = Array.isArray(surfaces[name]) ? surfaces[name] : [surfaces[name]];
          item.checked = expected.indexOf(item.value) >= 0;
        });
      } else field.value = surfaces[name];
    });
  }

  function appendFeedbackRow(list, label, surface) {
    var wrapper = document.createElement("div");
    var term = document.createElement("dt");
    var description = document.createElement("dd");
    term.textContent = label;
    description.textContent = surface;
  ...
  function renderGuidedFeedback(container, result) {
    ...
    container.evidenceset.state = result.pass ? "pass" : "gap";
    container.replaceChildren();
    var title = document.createElement("h3");
    title.textContent = result.title;
    var summary = document.createElement("p");
    summary.textContent = result.pass
      ? "你的选择抓住了本模块的第一个产品决策。继续看完整判断链，确认它能迁移到其他形态判断。"
    ...
    appendFeedbackRow(list, "换项目后保持不变", result.definition.invariant);
    container.append(title, summary, list);
  }

  function evaluateGuided(id, surfaces) {
    var definition = guidedDefinitions[id] || {};
    var choiceCorrect = surfaces.choice === definition.correct;
    return {
      pass: choiceCorrect,
      title: choiceCorrect ? definition.passTitle : definition.failTitle,
  ...
  function setupGuidedPractices() {
    document.querySelectorAll("[evidence-guided-practice]").forEach(function (form) {
      var id = form.getAttribute("evidence-guided-practice");
      var moduleState = getModuleState(id);
      var feedback = document.querySelector('[evidence-guided-feedback="' + id + '"]');
      form.addEventListener("submit", function (event) {
        event.preventDefault();
        var result = evaluateGuided(id, surfacesOf(form));
        moduleState.guided = { completed: result.pass, result: result, surfaces: result.surfaces, updated_at: result.updated_at };
        renderGuidedFeedback(feedback, result);
        refreshProgressAndHandoff();
        saveState(result.pass ? "关键判断已辨清；继续完成立项口述。" : "解析已展开；修改后可以再次提交。");
      });
      if (moduleState.guided && moduleState.guided.surfaces) {
        restoreFormValues(form, moduleState.guided.surfaces);
        renderGuidedFeedback(feedback, moduleState.guided.result || evaluateGuided(id, moduleState.guided.surfaces));
      }
    });
  }

  function setTransferFeedback(id, completed, message) {
    var container = document.querySelector('[evidence-transfer-feedback="' + id + '"]');
    if (!container) return;
    container.hidden = false;
    container.evidenceset.state = completed ? "pass" : "gap";
    var paragraph = container.querySelector("p");
    if (paragraph && message) paragraph.textContent = message;
    var reveal = container.querySelector("[evidence-reveal-reference]");
    if (reveal) reveal.hidden = !completed;
  }

  function setupTransferPractices() {
    document.querySelectorAll("[evidence-transfer-practice]").forEach(function (form) {
      var id = form.getAttribute("evidence-transfer-practice");
      var moduleState = getModuleState(id);
      form.addEventListener("submit", function (event) {
        event.preventDefault();
        ...
        setTransferFeedback(id, completed, completed
          ? "答辩记录已记录。打开示例后，挑一处补得更具体。"
          : "再补一层：写清本周动作、验收物和失败时下一步。");
        refreshProgressAndHandoff();
        saveState(completed ? "答辩记录已记录。" : "答辩记录还需要补充。");
      });
  ...
  function setupReferenceToggles() {
    document.querySelectorAll("[evidence-reveal-reference]").forEach(function (button) {
      button.addEventListener("click", function () {
        var id = button.getAttribute("evidence-reveal-reference");
        var panel = document.querySelector('[evidence-reference-panel="' + id + '"]');
        if (!panel) return;
        var open = panel.hidden;
        panel.hidden = !open;
        button.setAttribute("aria-expanded", String(open));
        button.textContent = open ? "收起示例" : (id === "final" ? "查看示例制定稿" : "打开示例方案");
      });
    });
  }

  function setTeachbackStatus(id, completed, message) {
    var status = document.querySelector('[evidence-teachback-status="' + id + '"]');
    if (!status) return;
    status.evidenceset.state = completed ? "pass" : "gap";
    status.textContent = message;
  }

  function setupTeachbacks() {
    document.querySelectorAll("[evidence-teachback-form]").forEach(function (form) {
      var id = form.getAttribute("evidence-teachback-form");
      var moduleState = getModuleState(id);
      form.addEventListener("submit", function (event) {
        event.preventDefault();
        var field = form.querySelector("textarea");
  ...
  function normaliseChecks(surface) {
    if (!surface) return [];
    return Array.isArray(surface) ? surface : [surface];
  }

  function setupFinalChallenge() {
    var form = document.querySelector("[evidence-final-challenge]");
    if (!form) return;
    var error = document.querySelector("[evidence-final-error]");
    var feedback = document.querySelector("[evidence-final-feedback]");
    if (state.final_practice) {
      normaliseChecks(state.final_practice.checks).forEach(function (surface) {
        var field = form.querySelector('input[name="final_check"][surface="' + surface + '"]');
    ...
    form.addEventListener("submit", function (event) {
      event.preventDefault();
      var surfaces = surfacesOf(form);
      var answer = String(surfaces.final_diagnosis || "").trim();
      var checks = normaliseChecks(surfaces.final_check);
      ...
      if (error) error.textContent = completed ? "" : "请沿六步路线补充 W01-D07 答辩记录，并完成五项覆盖核对。";
      if (feedback) feedback.hidden = !completed;
      refreshProgressAndHandoff();
      saveState(completed ? "W01-D07 答辩记录已保存；可以查看示例。" : "W01-D07 答辩记录还没有覆盖完整链路。");
    });
  }
  ...
  function updateProgress() {
    moduleOrder.forEach(function (id) {
      ...
      var status = document.querySelector('[evidence-module-status="' + id + '"]');
      if (status) {
        status.textContent = count + " / 2 步";
        status.evidenceset.state = count === 2 ? "complete" : count > 0 ? "progress" : "empty";
      }
    });
    ...
    var completedCount = Object.keys(control).filter(function (name) { return control[name]; }).length;
    var bar = document.querySelector("[evidence-ability-progress]");
    var text = document.querySelector("[evidence-ability-progress-text]");
    if (bar) bar.style.width = (completedCount / 4 * 100) + "%";
    if (text) text.textContent = completedCount + " / 4";
    Object.keys(control).forEach(function (name) {
      var item = document.querySelector('[evidence-ability-control="' + name + '"]');
      var label = document.querySelector('[evidence-ability-status="' + name + '"]');
      if (item) item.evidenceset.state = control[name] ? "complete" : "pending";
      if (label) label.textContent = control[name] ? "已完成" : (name === "explain" ? "未探索" : "未完成");
    });
  }
  ...
  function answerCardMarkdown() {
    ...
    var lines = ["# 我的 W01-D07 AI 产品立项口述理解地图", ""];
    moduleOrder.forEach(function (id) {
      lines.push("## " + (moduleLabels[id] || id), answer(id + "_teachback", "还没有记录这一模块的学习笔记"), "");
    });
    lines.push("## W01-D07 答辩记录", answer("final_diagnosis", "还没有完成 W01-D07 综合工作坊"), "");
    lines.push("> 这是网页学习草稿，不是正式掌握证据；疑问和未定项应保留到 Codex 正式接力。");
    return lines.join("\n");
  }

  function renderAnswerCard(surface) {
    var panel = document.querySelector("[evidence-answer-card]");
    var output = document.querySelector("[evidence-answer-card-output]");
    var toggle = document.querySelector("[evidence-toggle-answer-card]");
    var copy = document.querySelector("[evidence-copy-answer-card]");
    if (output) output.textContent = surface;
    if (panel) panel.hidden = false;
    if (toggle) { toggle.disabled = false; toggle.textContent = "收起理解地图"; }
    if (copy) copy.disabled = false;
  }

  function copyText(surface, message) {
    function fallback() {
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) navigator.clipboard.writeText(surface).then(function () { announce(message); }).catch(fallback);
    else fallback();
  }

  function setupAnswerCard() {
    var build = document.querySelector("[evidence-build-answer-card]");
    var toggle = document.querySelector("[evidence-toggle-answer-card]");
    var copy = document.querySelector("[evidence-copy-answer-card]");
    var panel = document.querySelector("[evidence-answer-card]");
    if (build) build.addEventListener("click", function () {
      ...
      saveState("W01-D07 理解地图已由你的笔记和答辩记录汇总。");
    });
  ...
  function setupActions() {
    var clear = document.querySelector("[evidence-clear-session-draft]");
    ...
    document.querySelectorAll("[evidence-copy-formal-entry]").forEach(function (button) {
      button.addEventListener("click", function () {
        copyText(JSON.stringify(window.__STUDY_PAGE_HANDOFF__ || {}, null, 2), "W01-D07 学习接力已复制。");
      });
    });
  }

  // W01-D07 · Agent Shape Selection marker for maintenance and verification.

  function setupSectionNavigation() {
    var sections = Array.prototype.slice.call(document.querySelectorAll("[evidence-chapter-section]"));
    var links = Array.prototype.slice.call(document.querySelectorAll("[evidence-section-link]"));
    ...
    function update(forceReveal) {
      ...
      links.forEach(function (link) {
        if (link.getAttribute("evidence-section-link") === active.id) {
  ...
  if (state.migrated_legacy_key) saveState("旧 W01-D07 草稿已迁移到新的学习闭环；原数据未删除。");
})();
