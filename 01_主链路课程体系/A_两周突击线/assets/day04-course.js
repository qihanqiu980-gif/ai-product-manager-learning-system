(function () {
  ...
  var guidedDefinitions = {
    control: {
      correct: "program_guard",
      passTitle: "模型候选回到了确定性程序的许可边界。",
      failTitle: "格式正确或模型重复判断仍被误当成执行权限。",
      known: "模型返回合法 JSON，但会话次数已经达到合规上限。",
      error: "把输出格式正确当成业务流转合法，跳过程序掌握的计数与规则。",
      why: "模型可以理解语义，却不能可靠持有所有确定性状态，也不承担状态写入和恢复。",
      chain: "接收模型候选 → 程序读取计数与合规规则 → 拒绝越界 → 记录原因与合法下一步。",
      invariant: "模型只提出概率性候选；硬约束、写入、日志和恢复必须由确定性程序控制。"
    },
    transition: {
      correct: "validation_event",
      passTitle: "刚发生的事件与程序读取的状态快照已经分开。",
      failTitle: "题号、预算或模型建议仍被误写成事件。",
      known: "刚完成的是回答结构校验；题号和剩余预算在此之前就存于状态中。",
      error: "把当前有什么数据混进刚发生了什么，导致流转无法稳定触发和回放。",
      why: "Event 是离散触发；题号、计数和覆盖属于 Guard 的判断输入。",
      chain: "answer_validation_passed → Guard 读取状态与预算 → 执行动作 → 写入正式 Next State。",
      invariant: "Event、Guard、Action、Next State 必须能独立记录、测试和恢复。"
    },
    coverage: {
      correct: "asked_pending",
      passTitle: "覆盖快照忠实反映了“已发题、未回答”这一时间点。",
      failTitle: "流程状态、证据深度或预算仍被提前改写。",
      known: "问题已经发出，生成前预算为 2，但学习者还没有回答。",
      error: "把出题动作直接写成已覆盖或充分证据，或忘记扣减已经消耗的题目预算。",
      why: "Coverage 管理证据收集进度，不是能力百分比分；证据只能来自真实回答和验证。",
      chain: "锁定时间点 → asked → pending_evaluation → 记录 policy-q12 → budget 从 2 减到 1。",
      invariant: "流程进度、证据深度、稳定引用和剩余预算必须分别按真实事件更新。"
    },
    stop: {
      correct: "limited",
      passTitle: "流程必须结束，但不完整覆盖没有被伪装成正常完成。",
      failTitle: "题目耗尽仍被误解成 completed 或被允许超预算继续。",
      known: "预算已耗尽；一项有有效证据，另一项未覆盖；保存与校验正常。",
      error: "把“不能继续”直接等同于“完整交付条件全部通过”。",
      why: "completed 要求题数、覆盖、保存和校验同时通过；超预算继续又破坏确定性规则。",
      chain: "核对异常/退出 → 不满足 completed → 至少有有效证据 → limited_report + 明确未覆盖限制。",
      invariant: "流程必须结束与结果可以完整交付永远是两件事。"
    },
    degradation: {
      correct: "stop_block",
      passTitle: "安全降级先阻断影响，再进入留证和恢复。",
      failTitle: "修改 Prompt、只记摘要或继续交付都跳过了立即止损。",
      known: "校验连续失败达到上限，未校验数据尚未写入财务下游。",
      error: "在结果仍不可信时继续自动重试或交付，让故障影响扩大。",
      why: "修复原因需要时间；第一动作必须先阻断不可信结果和无限自动处理。",
      chain: "停止重试与下游写入 → 告警 → 冻结完整现场 → 人工或隔离恢复 → 重新校验后放行。",
      invariant: "所有安全降级都必须包含止损、失败现场、受控恢复和重新放行条件。"
    }
  };
  ...
  function applyFormalState(stateSource) {
    ...
    var completed = day04.formal_status === "completed";
    var mastered = day04.concept_status === "mastered";
    var activeLabel = formal.day_id === "day-07" && day07.formal_status === "in_progress"
      ? "Day 07 正式课程进行中"
      : "当前正式位置：" + (formal.day_id || "未识别");
    var passed = "Day 04 已正式验证：状态机四要素与模型 / 程序边界、覆盖流程状态与预算、有限报告归因、停止分支，以及安全降级的止损、留证和恢复动作。";
    var reviewBoundary = mastered
      ? "机制解释、应用与跨场景迁移均已 verified；保持层为 scheduled，既有计划包含 2026-08-25 与 2026-09-10。提前结束二次确认细节仍延期到长期线。"
      : "Day 04 当前掌握状态为 " + (day04.concept_status || "unknown") + "；仍以正式记录为准。";
    var task = formal.pending_question_id || formal.required_retest_of_question_id || (formal.day_id === "day-07" ? "继续 Day 07 网页课程；Day 04 无待答题或复测关系" : "按当前正式状态源继续");
    var surface = "网页 · Day 04 已掌握复盘";
    var condition = "完成本地复盘后返回 " + activeLabel + "；本页不重复正式测试已掌握范围，只有你明确提出时才安排新的正式复测。";

    setText("[data-later-page-status]", completed ? "Day 04 课程已完成 · mastery = " + (day04.concept_status || "unknown") : "Day 04 状态需核对 · 本页不伪造完成");
    setText("[data-later-formal-summary]", "Day 04 " + (day04.formal_status || "unknown") + " / " + (day04.concept_status || "unknown") + "；" + activeLabel + "；历史 session = " + (day04.session_id || "无") + "。");
    setText("[data-later-cockpit-title]", mastered ? "Day 04 已正式掌握：按需复盘，不重复证明。" : "Day 04 正式记录已读取：本页不改写历史状态。");
    setText("[data-later-cockpit-summary]", "Day 04 正式会话 " + (day04.session_id || "历史会话") + " 已结束且没有 pending question。网页复盘只写当前标签页，不创建 session、评分、错题、复测关系或掌握证据。");
    ...
    setText("[data-formal-handoff-title]", "Day 04 已正式掌握：本页只用于复盘和保持。" );
    setText("[data-formal-handoff-summary]", "网页复盘不会重启已结束的 Day 04 会话，也不会覆盖机制、应用和迁移的正式证据。当前正式主线是 " + activeLabel + "。" );

    var stageState = {
      foundation: day03.concept_status === "mastered" ? "verified" : "gap",
      day04: mastered ? "verified" : completed ? "gap" : "preview",
      day05: day05.concept_status === "needs_reinforcement" ? "gap" : day05.formal_status === "completed" ? "verified" : "preview",
    ...
    setText('[data-later-stage-status="foundation"]', day03.concept_status === "mastered" ? "前序正式完成 · mastered" : "以正式记录为准");
    setText('[data-later-stage-status="day04"]', completed ? "课程完成 · " + (day04.concept_status || "unknown") : "未完成");
    setText('[data-later-stage-status="day05"]', day05.formal_status === "completed" ? "课程完成 · " + (day05.concept_status || "unknown") : "未完成");
  ...
  function answerCardMarkdown() {
    ...
    return [
      "# 我的 Day 04 状态机与能力覆盖理解地图", "",
      "## 模型建议与程序控制：我的理解或疑问", answer("control_teachback", "尚未记录"), "",
      "## 状态流转四要素：我的理解或疑问", answer("transition_teachback", "尚未记录"), "",
      "## 能力覆盖时间点：我的理解或疑问", answer("coverage_teachback", "尚未记录"), "",
      "## 停止分支与有限报告：我的理解或疑问", answer("stop_teachback", "尚未记录"), "",
      "## 安全降级与恢复：我的理解或疑问", answer("degradation_teachback", "尚未记录"), "",
      "## 综合控制工作坊", answer("final_diagnosis", "尚未完成工作坊"), "",
      "> 这是 Day 04 本地复盘痕迹，不是新一次正式评测，不会覆盖既有 mastered 证据。"
    ].join("\n");
  }
  ...
  function setupActions() {
    ...
    if (clear) clear.addEventListener("click", function () {
      if (!window.confirm("只清除当前标签页的 Day 04 复盘练习与草稿？正式记录和旧 localStorage 数据不会被删除。")) return;
  ...
  if (state.migrated_legacy_key) saveState("旧 Day 04 草稿已只读迁移到新的复盘闭环；原数据未删除。" );
})();
